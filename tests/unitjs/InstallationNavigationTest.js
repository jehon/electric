...
webDescribe("InstallationNavigationTest.js", "<installation-navigation></installation-navigation>", function(element) {
	webDescribe("with a target", "<div id='tab_target'></div>", function(tab_target) {
		beforeEach(function() {
		...
		it("should load", function() {
			expect(element().tagName.toLowerCase()).toBe("installation-navigation");
		});
		...
		describe("with initialized installation", function() {
			...
			it("shoud select by click on plan", function() {
				...
				expect(tab_target().querySelectorAll("installation-filiaire").length).toBe(0);
				expect(tab_target().querySelectorAll("installation-position").length).toBe(1);
			});

			it("shoud select by click on plan", function() {
				clickTo(element().querySelector("#filiaire"));
				expect(element().querySelectorAll(".active").length).toBe(1);
				expect(element().querySelectorAll("#filiaire.active").length).toBe(1);
				expect(tab_target().querySelectorAll("installation-filiaire").length).toBe(1);
				expect(tab_target().querySelectorAll("installation-position").length).toBe(0);
			});

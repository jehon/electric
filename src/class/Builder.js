...
class Builder {
	...
	_getBuilder(element, isDirect, index) {
		...
		return new this.constructor(element).build(this.getDescendantBuildParameters({ isDirect, index }));
	}
	...
	build() {
		...
		if (this._currentElement.next) {
		    next = this._currentElement.next.map((e, i) => this._getBuilder(this._currentElement.next[i], true, i));
		}

		// Build for alternate
		let alternate = [];
		if (this._currentElement.alternate) {
		    alternate = this._currentElement.alternate.map((e, i) => this._getBuilder(this._currentElement.alternate[i], false, i));
		}
